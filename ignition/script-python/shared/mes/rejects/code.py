...
def buildRejectStructure(machineName, ignitionTagPath, inputFamilyId, wasteDeclarationType = "Single"):	
	"""
	...
	for row in system.dataset.toPyDataSet(sqlResult):
		...
		if row["Family Id"] > 0 and inputFamilyId == 0:
			if row["Family Id"] not in family:
				leveled = 1
				wasteDesc = system.db.runScalarPrepQuery("SELECT category FROM factory_reject_families WHERE id = ?", [row["Family Id"]], "factory_parameters")
				family.append(row["Family Id"])
				displayText = wasteDesc
			...
			wasteDesc = row["Description"]
			
			if row["Local Description"] is not None:
				if len(row["Local Description"]) > 0: 
					wasteDesc = row["Local Description"]

			displayText = "<html><font size='6'><b>" + row["Reject Code"] + "</b></font><br>" + wasteDesc + "</html>"
			appendData = 1
